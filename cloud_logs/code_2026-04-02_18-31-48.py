...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float):
        ...
        self.c_qkv = AnnealedBitLinear(dim, self.q_dim + 2 * self.kv_dim, bias=False)
    ...
    def forward(self, x: Tensor, step_fraction: Tensor) -> Tensor:
        ...
        qkv = self.c_qkv(x, step_fraction)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_unique_blocks: int, model_dim: int, num_heads: int,
        ...
        for i in range(num_layers):
            ...
            block.attn.c_qkv.weight_latent = leader.attn.c_qkv.weight_latent
