...
class AnnealedBitLinear(nn.Module):
    ...
    @torch.no_grad()
    def precompute_delta(self, step_fraction: Tensor) -> None:
        w = self.weight_latent
        w_abs = w.abs()
        scale = w_abs.mean(dim=1, keepdim=True).clamp(min=1e-8)
        w_quant = torch.sign(w) * (w_abs > 0.7 * scale).float() * scale
        self._cached_delta = step_fraction * (w_quant - w)

    def forward(self, x: Tensor, step_fraction: Tensor) -> Tensor:
        W = self.weight_latent + self._cached_delta
...
def main() -> None:
    ...
    if args.warmup_steps > 0:
        ...
        for module in raw_model.modules():
            if isinstance(module, AnnealedBitLinear):
                module.precompute_delta(sf_zero)
    ...
    while True:
        ...
        # --- HOLD-COSINE LR SCHEDULE (no warmup) ---
        loop_elapsed_ms = training_time_ms + 1000.0 * (time.perf_counter() - t0)
        fraction = min(loop_elapsed_ms / max(loop_max_ms, 1.0), 1.0)
        hold_end = 0.70
        min_lr_scale = 0.01
        if fraction < hold_end:
            scale = 1.0
        else:
            decay_progress = (fraction - hold_end) / (1.0 - hold_end)
            scale = min_lr_scale + (1.0 - min_lr_scale) * 0.5 * (1.0 + math.cos(math.pi * decay_progress))
