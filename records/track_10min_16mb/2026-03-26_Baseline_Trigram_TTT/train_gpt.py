"""Baseline architecture (9L MLP2x) + TrigramHash + Test-Time Training"""
...
def eval_val(
    ...
    if ttt and base_model is not None:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.trigram = TrigramHash(trigram_buckets, trigram_dim, model_dim) if trigram_enabled else None
...
def main() -> None:
    ...
    if base_model.trigram is not None:
        tok_params.append(base_model.trigram.embed.weight)
    ...
    log0(f"ttt_enabled:{args.ttt_enabled} ttt_lr:{args.ttt_lr} ttt_steps:{args.ttt_steps}")
    ...
    torch.cuda.synchronize()
    ttt_tag = " ttt:enabled" if use_ttt else ""
    log0(
